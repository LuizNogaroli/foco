...
const selectStartRegex = /<select[^>]*id="campo511"[^>]*>/;
const match = content.match(selectStartRegex);
...
if (match) {
  ...
  if (endIndex > -1) {
    ...
    console.log('Successfully replaced all options inside campo511 HTML.');
  } else {
    console.log('Could not find </select> for campo511');
  }
} else {
  console.log('Could not find <select id="campo511">');
}
