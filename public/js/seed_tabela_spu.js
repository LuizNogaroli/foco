    ...
    for (const [idx, req] of reqs.entries()) {
        const payload = {
            ...
            dados_json: {
                ...
                natureza: (idx % 4 === 3) ? "Rural" : "Urbano",
                tipo_imovel: ["Lote/Terreno", "Gleba", "Ilha", "Outros"][idx % 4],
                classificacao: ["Dominial", "Especial", "Uso Comum"][idx % 3],
                inscricao_municipal: (idx % 4 === 3) ? "" : `Inscrição Municipal ${req.num}-${idx}`,
                ccir: (idx % 4 === 3) ? `CCIR ${req.num}-${idx}` : "",
                cep: "70000-000",
                uf: req.uf,
                municipio: req.mun,
